...
$vm1 = Get-Template -Name 'vm-template-1'
$vm2 = Get-Template -Name 'vm-template-2'
$vm3 = Get-Template -Name 'vm-template-3'
$vm4 = Get-Template -Name 'vm-template-4'
...
ForEach ($user in $studentUsers)
{
    ...
    $VMName = "ClassID-" + $username + "-" + $VMName # You can change VMName, to not append template name
    New-VM -Name $VMName -Template $app_template -VMHost $VMHost -Datastore $Datastore
    Start-VM –VM $VMName

    $VMName = "ClassID-" + $username + "-" + $VMName
    New-VM -Name $VMName -Template $backups_template -VMHost $VMHost -Datastore $Datastore
    Start-VM –VM $VMName  

    $VMName = "ClassID-" + $username + "-" + $VMName
    New-VM -Name $VMName -Template $db_template -VMHost $VMHost -Datastore $Datastore
    Start-VM –VM $VMName  

    $VMName = "ClassID-" + $username + "-" + $VMName
    New-VM -Name $VMName -Template $mgmt_template -VMHost $VMHost -Datastore $Datastore
    Start-VM –VM $VMName  
