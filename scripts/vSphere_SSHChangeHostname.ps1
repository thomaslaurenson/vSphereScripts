...
<#
# Commented: Existing keyword search for VMs
...
# Read hostnames from a 
$vms = @()

# CSV file to import
# Script expects first row to be a header (named "USERNAME")
# The CSV only has one column of usernames
$csvFile = $pwd + "/input.csv" # CHANGE: to actual input file
...
Foreach ($user in $studentUsers) {
    ...
    $vmname = "vm-basename" + $user.USERNAME # CHANGE: to actual VM basename
    $vms += $vms = Get-VM -Name $vmname
}
...
$username = 'myusername' # CHANGE: to actual username
$password = 'mypassword' # CHANGE: to actual password
...
$scriptToRun = "$scriptHelperDirectory/scriptChangeHostname.sh" 
...
foreach($vm in $sorted_vms){
    ...
    Start-Process ($plinkCmd) `
        -ArgumentList ("-P $sshPort $username@${VMIP4} -batch -pw $password -m ${scriptHelperDirectory}hostnameCmd") `
        -RedirectStandardOutput $outFile -RedirectStandardError $errFile -NoNewWindow -Wait 
